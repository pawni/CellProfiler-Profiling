...
class memoized(object):
    ...
    def __call__(self, *args):
        ...
        try:
            value = cpf.util.unpickle(filename)[0]
            #logger.debug('Using cached value')
            return value
        except IOError:
            value = self.func(*args)
            cpf.util.pickle(filename, value)
            return value
    def __repr__(self):
...
def images_by_plate(filter):
    f = cpf.properties._filters[filter]
    d = {}
...
def profile_svmnormalvector(cache_dir, group_name, control_filter, 
        ...
        group, colnames_group = cpf.db.group_map(group_name, reverse=True, 
                                                 filter=filter)
